...
export const oauthCallback = asyncHandler(async (req, res) => {
  const backTo = (params) =>
    res.redirect(`/emails?${new URLSearchParams(params)}`);

  // The user denied consent, or Google returned an error.
  if (req.query.error)
    return backTo({ google: "error", reason: String(req.query.error) });

  const { code, state } = req.query;
  if (!code || !state)
    return backTo({ google: "error", reason: "missing_code" });
  ...
  if (claims.sa !== true)
    return backTo({ google: "error", reason: "forbidden" });
  ...
  try {
    ...
    return backTo({
      google: "error",
      reason: err.message || "exchange_failed",
    });
  }

  // On re-consent Google may omit the refresh token; keep the previously stored one.
  const existing = await EmailAccount.findOne({ key: ACCOUNT_KEY }).select(
    "+refreshToken",
  );
...
export const send = asyncHandler(async (req, res) => {
  ...
  const jobs = recipients.map((r, i) => {
    ...
    if (!EMAIL_RE.test(email))
      throw ApiError.badRequest(
        `כתובת מייל לא תקינה בשורה ${i + 1}: "${email}"`,
      );
  ...
  const acct = await EmailAccount.findOne({ key: ACCOUNT_KEY }).select(
    ...
    for (;;) {
      try {
        ...
        results.push({
          email: job.email,
          ok: false,
          error: err.message || "שגיאה בשליחה",
        });
        if (err.fatalAuth) {
          aborted = true;
...
export const signedContracts = asyncHandler(async (req, res) => {
  const regs = await Registration.find({ "contract.status": "signed" })
    ...
    .select(
      "studentName student courseRaw contract.signedAt contract.emailedAt contract.emailedTo",
    )
    .lean();
  ...
  const [pdfs, students] = await Promise.all([
    ContractPdf.find({ registration: { $in: ids } })
      .select("registration")
      .lean(),
  ...
  const emailById = new Map(
    students.map((s) => [String(s._id), s.email || ""]),
  );
...
export const resendSignedContracts = asyncHandler(async (req, res) => {
  ...
  const ids = Array.isArray(req.body?.registrationIds)
    ? req.body.registrationIds
    : [];
  if (!all && ids.length === 0)
    throw ApiError.badRequest("יש לבחור לפחות חוזה אחד");
  ...
  if (regs.length > 300)
    throw ApiError.badRequest("יותר מדי חוזים בבת אחת (עד 300)");
  ...
  for (const reg of regs) {
    ...
    try {
      await sendOneEmail({
        ...
        attachments: [
          {
            filename: pdfDoc.filename || "contract.pdf",
            mimeType: "application/pdf",
            base64: pdfDoc.pdfBase64,
          },
        ],
      });
      ...
      results.push({
        ...label,
        ok: false,
        reason: err.notConnected ? "not_connected" : err.message,
      });
