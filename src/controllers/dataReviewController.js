...
const repFilter = (req) => (req.user?.role === 'rep' ? { rep: req.user._id } : {});
...
function studentIssues(student) {
  ...
  if (id && !isValidIsraeliId(id)) {
    issues.push({ field: 'realIdNumber', label: 'ת.ז. אינה תקינה (ספרת ביקורת)' });
  }
  return issues;
}
...
const expectedPaymentsSum = (reg) => {
  ...
  return { total, expected: round2(Math.max(total - (Number(reg.writeOff) || 0), 0)) };
};
...
function dealIssues(reg) {
  ...
  if (is2026) {
    if (payments.length === 0) issues.push({ field: 'payments', label: 'חסר פירוט תשלומים' });
    else {
      ...
      if (payments.some((p) => !PAY_METHODS.includes(p.methodCategory || p.method))) {
        issues.push({ field: 'payments', label: 'יש תשלום ללא אמצעי תשלום מוגדר' });
      }
    }
  }
  if (!reg.cohort) issues.push({ field: 'cohort', label: 'לא שויך מחזור קורס' });
  return issues;
}
...
function suggestPayments(reg) {
  ...
  const total = round2(reg.dealPrice > 0 ? reg.dealPrice : reg.totalAmount);
  if (!(total > 0)) return [];
  const method = PAY_METHODS.includes(reg.paymentCategory) ? reg.paymentCategory : 'credit';
  ...
  const remaining = round2(total - (advance > 0 && advance < total ? advance : 0));
  if (remaining > 0) {
    ...
    if (method === 'credit' || n <= 1) {
      ...
      const base = reg.nextPaymentDate ? new Date(reg.nextPaymentDate) : new Date(reg.dealDate);
      ...
      let creditLeft = Math.max(round2(reg.totalPaid) - (advance > 0 && advance < total ? advance : 0), 0);
      for (let i = 0; i < n; i += 1) {
...
async function suggestCohort(reg, cohorts) {
  ...
  const bySource = reg.course
    ...
    : null;
  if (bySource) return { cohortId: String(bySource._id), reason: 'לפי הקורס שקושר בייבוא' };
  ...
  const hit = cohorts.find((c) => {
    ...
    const nameMatch = cname && raw && (raw.includes(cname) || cname.includes(raw));
    const labelMatch = clabel && label && clabel === label;
    return nameMatch && (labelMatch || !label);
  });
  return hit ? { cohortId: String(hit._id), reason: 'לפי שם הקורס/מחזור' } : null;
}
...
async function buildDeals(filter) {
  const regs = await Registration.find(filter)
    ...
    .sort({ dealDate: -1 })
    .lean();
  const studentIds = [...new Set(regs.map((r) => r.student).filter(Boolean).map(String))];
  ...
  for (const reg of regs) {
    ...
    data.push({
      ...
      issues: [...sIssues.map((i) => ({ ...i, scope: 'student' })), ...dIssues.map((i) => ({ ...i, scope: 'deal' }))],
      source: {
...
export const listDeals = asyncHandler(async (req, res) => {
  ...
  res.json({
    success: true,
    data,
    summary: { total: data.length, done, pending: data.length - done, shown: data.length },
  });
});
...
export const listCohortOptions = asyncHandler(async (req, res) => {
  ...
  res.json({
    ...
    data: cohorts.map((c) => ({
      ...
      firstSession: (c.sessions || []).map((s) => s.date).sort((a, b) => new Date(a) - new Date(b))[0] || null,
    })),
  });
});
...
export const dealSource = asyncHandler(async (req, res) => {
  const reg = await Registration.findOne({ _id: req.params.id, ...repFilter(req) }).lean();
  ...
  const rows = [...new Set([reg.sourceRow, ...refs.map((r) => r.sourceRow)].filter(Boolean))];
  if (!file || rows.length === 0) {
    return res.json({ success: true, data: null, message: 'לעסקה זו אין הפניה לקובץ מקור' });
  }
  ...
  if (docs.length) {
    ...
    if (header.some((c) => isJunk(c.v))) {
      ...
      if (other) {
        const alt = await SourceRow.findOne({ file: other, row: 1 }).select('cells').lean();
        if (alt?.cells?.length) {
          header = header.map((c, i) => (isJunk(c.v) && alt.cells[i] && !isJunk(alt.cells[i].v) ? alt.cells[i] : c));
        }
      }
    }
...
export const updateStudent = asyncHandler(async (req, res) => {
  ...
  if (req.user?.role === 'rep') {
    const owns = await Registration.exists({ student: student._id, rep: req.user._id });
  ...
  if (id && !isValidIsraeliId(id)) {
    throw ApiError.badRequest('מספר תעודת הזהות אינו תקין (ספרת הביקורת אינה מתאימה)');
  }
  const email = cleanStr(b.email).toLowerCase();
  if (email && !/^[^\s@]+@[^\s@]+\.[^\s@]{2,}$/.test(email)) {
    throw ApiError.badRequest('כתובת המייל אינה תקינה');
  }
  if (b.gender !== undefined && b.gender && !['male', 'female'].includes(b.gender)) {
    throw ApiError.badRequest('מין לא תקין');
  }
  if (b.title !== undefined && b.title && !['Mr.', 'Ms.', 'Mrs.'].includes(b.title)) {
    throw ApiError.badRequest('פנייה לא תקינה');
  }

  const FIELDS = [
    'firstName', 'lastName', 'englishName', 'hebrewName',
  ...
  await student.save();
  res.json({ success: true, data: { id: String(student._id), fullName: student.fullName } });
});
...
export const updatePayments = asyncHandler(async (req, res) => {
  const reg = await Registration.findOne({ _id: req.params.id, ...repFilter(req) });
  if (!reg) throw ApiError.notFound('העסקה לא נמצאה');
  ...
  const payments = raw.map((p, i) => {
    ...
    if (!method) throw ApiError.badRequest(`אמצעי תשלום לא תקין בתשלום ${i + 1}`);
    ...
    return {
      ...
      dueDate,
      paid,
      installments: Number(p.installments) > 1 ? Number(p.installments) : undefined,
      note: cleanStr(p.note) || undefined,
      confirmedByName: paid ? `${req.user?.name || ''} (עריכת נתונים)` : undefined,
  ...
  if (req.body?.dealPrice !== undefined && req.user?.role === 'manager') {
    ...
    if (!Number.isFinite(n) || n <= 0) throw ApiError.badRequest('מחיר עסקה אינו תקין');
    declaredTotal = n;
  }
  const hadExplicitPrice = reg.dealPrice > 0;
  const total = declaredTotal ?? round2(hadExplicitPrice ? reg.dealPrice : reg.totalAmount);
  ...
  else if (!hadExplicitPrice && reg.schemaVersion !== 2 && total > 0) reg.dealPrice = total;
  // מעבר לפורמט v2: הכסף נגזר מהתשלומים
  reg.schemaVersion = 2;
  // אמצעי התשלום הראשי = הדומינטי (כמו ב-registrationController), לא הראשון ברשימה
  const byMethod = payments.reduce((m, p) => ({ ...m, [p.method]: (m[p.method] || 0) + 1 }), {});
  reg.paymentCategory = Object.entries(byMethod).sort((a, b) => b[1] - a[1])[0][0];
...
export const assignCohort = asyncHandler(async (req, res) => {
  ...
  const cohort = await CourseCohort.findById(cohortId).populate('catalogCourse', 'name');
  ...
  await reg.save();
  res.json({ success: true, data: { id: String(reg._id), cohort: String(cohort._id) } });
});
...
export const approveDeal = asyncHandler(async (req, res) => {
  ...
  if (req.body?.undo) {
    ...
    await reg.save();
    return res.json({ success: true, data: { id: String(reg._id), status: 'pending' } });
  }

  const student = reg.student ? await Student.findById(reg.student).lean() : null;
  const issues = [
    ...studentIssues(student).map((i) => i.label),
    ...dealIssues(reg.toObject()).map((i) => i.label),
  ];
  if (issues.length) {
    throw ApiError.badRequest(`לא ניתן לאשר — חסרים נתונים: ${issues.join('; ')}`);
  }
