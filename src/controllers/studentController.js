...
export const list = asyncHandler(async (req, res) => {
  ...
  const pipeline = [
        ...
        partialCount: {
          $size: {
            $filter: {
              input: "$deals",
              as: "d",
              cond: { $eq: ["$$d.paymentStatus", "partial"] },
            },
          },
        },
        unpaidCount: {
          $size: {
            $filter: {
              input: "$deals",
              as: "d",
              cond: { $eq: ["$$d.paymentStatus", "unpaid"] },
            },
          },
        },
        needsReview: {
    ...
    ...(req.query.state === "open"
      ? [{ $match: { totalOutstanding: { $gt: 0.5 } } }]
      : []),
    ...(req.query.state === "review"
      ? [{ $match: { needsReview: true } }]
      : []),
    ...(req.query.state === "settled"
      ? [
          {
            $match: { dealsCount: { $gt: 0 }, totalOutstanding: { $lte: 0.5 } },
          },
        ]
      : []),
