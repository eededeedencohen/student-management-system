...
export const paymentTasks = asyncHandler(async (req, res) => {
  ...
  for (const r of rows) {
    ...
    if (r.paid) {
      ...
      // הו"ק לא נכללת (owner 2026-08-27: "ERN מאושרים אוטומטית... אין מה לוודא") -
      // נשארים רק תשלומים שסומנו אוטומטית לפני 27/08 (אשראי/העברה/מזומן), קבוצה שתתרוקן.
      if (isAuto(r) && !isErnPayment(r))
