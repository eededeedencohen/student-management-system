...
export const exportRegistrations = asyncHandler(async (req, res) => {
  ...
  return sendWorkbook(res, { rows, sheetName: 'רישומים', fileName: 'registrations' });
});
...
export const exportCourseRoster = asyncHandler(async (req, res) => {
  ...
  if (isObjectId) {
    ...
    if (!courseField) throw ApiError.badRequest('נדרש מזהה קורס או courseField');
  ...
  const baseName = course?.name || req.query.courseField || decodeURIComponent(id);
...
export const exportByMonth = asyncHandler(async (req, res) => {
  ...
  const filter = { recordType: 'registration', dealDate: { $gte: from, $lt: to } };
  if (req.scopeRepId) filter.rep = req.scopeRepId;
  else if (req.query.repId) filter.rep = req.query.repId;
