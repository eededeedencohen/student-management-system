...
for (const file of ["מורן.xlsx", "מיכל.xlsx"]) {
  try {
    ...
    const ws =
      wb.Sheets[
        wb.SheetNames.find((n) => n === "924" || n === "גיליון1") ||
          wb.SheetNames[0]
      ];
...
for (const r of regs) {
  ...
  if (t > 0) {
    ...
    if (Math.abs(paid + out - t) > EPS)
      violations.inv.push({ r, expected: t - paid, got: out });
...
console.log(
  `• עסקאות ללא סכום במקור (total=0): ${total0}  (לא ניתן להחיל אינווריאנט)`,
);
...
show(
  violations.inv,
  (x) =>
    `${x.r.sourceFile}#${x.r.sourceRow} ${x.r.studentName}: total=${x.r.totalAmount} paid=${x.r.totalPaid} out=${x.r.outstanding} (צפוי ${x.expected})`,
);
console.log(`INV3  outstanding<=total      → הפרות: ${violations.over.length}`);
show(
  violations.over,
  (r) =>
    `${r.sourceFile}#${r.sourceRow} ${r.studentName}: out=${r.outstanding} > total=${r.totalAmount}`,
);
console.log(`INV2  ערכים שליליים           → הפרות: ${violations.neg.length}`);
console.log(
  `INV5  סטטוס תשלום עקבי         → הפרות: ${violations.status.length}`,
);
show(
  violations.status,
  (x) =>
    `${x.r.sourceFile}#${x.r.sourceRow} ${x.r.studentName}: status=${x.r.paymentStatus} (צפוי ${x.expStatus})`,
);
console.log(
  `BUG   תאים טקסטואליים עם יתרה שגויה → הפרות: ${violations.noisyBad.length}`,
);
show(
  violations.noisyBad,
  (x) =>
    `${x.r.sourceFile}#${x.r.sourceRow} ${x.r.studentName}: out=${x.r.outstanding} | תא מקור="${x.balCell}"`,
);
console.log("");
...
console.log(
  `  בדיקת איזון: נגבה+בחוץ = ${Math.round(sumPaid + sumOut).toLocaleString()} ₪  (צריך = סה"כ ${Math.round(sumTotal).toLocaleString()} ₪) → ${Math.abs(sumPaid + sumOut - sumTotal) < 5 ? "תקין ✔" : "פער!"}`,
);
