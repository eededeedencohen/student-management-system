...
const run = async () => {
  ...
  const existing = await CatalogCourse.find({})
    .select("name price defaultSessionCount")
    .lean();
  ...
  for (const [name, price, sessions] of COURSES) {
    ...
    if (found) {
      const same =
        found.price === price && found.defaultSessionCount === sessions;
      skipped.push(
