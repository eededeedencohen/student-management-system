...
const ensure = async ({ name, email, role, password, commission, aliases, superAdmin }) => {
  ...
  if (commission) user.commission = { ...user.commission?.toObject?.(), ...commission };
...
seeded.forEach((u) => console.log(`   ${u.role.padEnd(8)} | ${u.name.padEnd(12)} | ${u.email} | ${u.password}`));
