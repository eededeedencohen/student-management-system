...
console.log(
  `📖 students.json: ${data.students.length} סטודנטים, ${data.courses.length} קורסים, ` +
...
const parseSourceRef = (ref) => {
  ...
  return {
    file: `${m[1]}.xlsx`,
    sheet: SHEET_BY_FILE[m[1]] || undefined,
    row: parseInt(m[2], 10),
  };
};
...
function buildDeal(s, dl) {
  ...
  for (const p of dl.payments || []) {
    ...
    if (!METHOD_MAP[p.method])
      warn(`אמצעי תשלום לא מוכר בעסקה ${dl.dealId}: "${p.method}"`);
    ...
    if (p.type === "פריסה לתשלומים" && p.method === "ERN") {
      ...
      for (let i = 0; i < n; i += 1) {
        const isLast = i === n - 1;
        const amount =
          isLast && det.totalAmount
            ? Math.round((det.totalAmount - scheduled) * 100) / 100
            : per;
        scheduled += amount;
        payments.push({
          type: "installment",
          ...
          method: "ern",
          methodCategory: "ern",
          dueDate: det.firstPaymentDateISO
            ? addMonthsClamped(det.firstPaymentDateISO, i)
            : undefined,
          paid: confirmations[i] === true,
          installments: n,
          note: [
            `ERN ${i + 1}/${n}`,
            det.stoppedAfter != null && i >= det.stoppedAfter
              ? `⚠️ ההוראה הופסקה לאחר ${det.stoppedAfter}`
              : "",
            p.note || "",
            ...prov,
          ]
      ...
      const rcv = payments
        .filter((x) => x.method === "ern" && x.paid)
        .reduce((a, x) => a + x.amount, 0);
      if (
        det.amountReceivedToDate != null &&
        Math.abs(rcv - det.amountReceivedToDate) > 1
      ) {
        warn(
          `${dl.dealId}: ERN amountReceivedToDate=${det.amountReceivedToDate} אך סכום המאושרים=${rcv}`,
        );
      }
    } else {
      const det = p.details || {};
      ...
      payments.push({
        ...
        note: [
          isSpread && inst > 1 ? `פריסת אשראי ל-${inst} תשלומים` : "",
          p.note || "",
          ...prov,
        ]
          .filter(Boolean)
  ...
  const unpaidScheduled = payments
    .filter((x) => !x.paid)
    .reduce((a, x) => a + x.amount, 0);
  ...
  const reconciled =
    isCancelled ||
    expectedOut <= 0.5 ||
    Math.abs(unpaidScheduled - expectedOut) <= 1;
  return {
    payments,
    isCancelled,
    balance,
...
function checkDeal(s, dl, built, mismatches) {
  if (Math.abs(built.collected - (dl.totalPaid || 0)) > 1) {
    mismatches.push(
      `${dl.dealId} (${s.name}): totalPaid=${built.collected} JSON=${dl.totalPaid}`,
    );
  }
  // outstanding: a settled ("שולם") or cancelled deal must end with 0 open debt
  const jsonExpectedOut =
    dl.status === "שולם" || built.isCancelled ? 0 : Math.max(built.balance, 0);
  if (Math.abs(built.expectedOut - jsonExpectedOut) > 1) {
    mismatches.push(
      `${dl.dealId} (${s.name}): outstanding=${built.expectedOut} מצופה=${jsonExpectedOut}`,
    );
  }
  if (
    !built.isCancelled &&
    STATUS_HE_TO_EN[dl.status] &&
    built.expectedStatus !== STATUS_HE_TO_EN[dl.status]
  ) {
    mismatches.push(
      `${dl.dealId} (${s.name}): status=${built.expectedStatus} JSON=${dl.status}`,
    );
  }
}
...
try {
  ...
  for (const s of data.students)
    for (const dl of s.deals) checkDeal(s, dl, buildDeal(s, dl), dryMismatches);
  ...
  console.log(
    `✅ ריצה יבשה: כל ${data.students.reduce((a, s) => a + s.deals.length, 0)} העסקאות משוחזרות בדיוק (0 אי-התאמות) - ממשיכים`,
  );
  ...
  const backup = {
    ...
    expensesImported: await Expense.find({
      sourceFile: { $exists: true, $ne: null },
    }).lean(),
  };
  fs.writeFileSync(backupPath, JSON.stringify(backup));
  console.log(
    `💾 גיבוי נשמר: ${backupPath} (${backup.registrations.length} עסקאות, ${backup.students.length} סטודנטים)`,
  );
  ...
  for (const u of users)
    for (const k of [u.name, ...(u.aliases || [])])
      if (k) repByJsonName.set(normName(k), u);
  const resolveRep = (name) => repByJsonName.get(normName(name)) || null;
  for (const rn of new Set(
    data.students.flatMap((s) => s.deals.map((d) => d.rep)),
  )) {
  ...
  const courseByNorm = new Map(
    existingCourses.map((c) => [normName(c.name), c]),
  );
  ...
  for (const jc of data.courses) {
    ...
    if (found) {
      ...
      if (!found.cohortLabel && jc.cohort && jc.cohort !== "לא משובץ")
        set.cohortLabel = jc.cohort;
      ...
      const doc = await Course.create({
        ...
        cohortLabel:
          jc.cohort && jc.cohort !== "לא משובץ" ? jc.cohort : undefined,
        location: jc.location || undefined,
        status: /לא משובץ/.test(jc.name)
          ? "לא משובץ"
          : isOld
            ? "הסתיים"
            : "פעיל",
  ...
  data.students.forEach((s, i) =>
    studentByJsonId.set(s.id, insertedStudents[i]),
  );
  console.log(
    `👤 נוצרו ${insertedStudents.length} סטודנטים (studentNumber = מזהה JSON, כולל ת.ז. אמיתיות)`,
  );
  ...
  for (const s of data.students) {
    ...
    for (const dl of s.deals) {
      ...
      const noteEntries = [];
      for (const n of dl.notes || [])
        noteEntries.push({ text: n, date: dealDateObj, byName: "ייבוא JSON" });
      if (dl.cancellationNote)
        noteEntries.push({
          text: `ביטול: ${dl.cancellationNote}`,
          date: dealDateObj,
          byName: "ייבוא JSON",
        });
      if (built.writeOffNote)
        noteEntries.push({
          text: built.writeOffNote,
          date: dealDateObj,
          byName: "ייבוא JSON",
        });
      ...
      for (const r of reasons) {
        noteEntries.push({
          text: `${isHigh ? "לבדיקה" : "הנחה מתועדת"}: ${r}`,
          date: dealDateObj,
          byName: "ייבוא JSON",
        });
      }
      ...
      const maxInst = Math.max(
        1,
        ...built.payments.map((x) => x.installments || 1),
      );
      ...
      const reg = new Registration({
        ...
        reconcileNote: built.reconciled
          ? undefined
          : `יתרה של ₪${built.expectedOut.toLocaleString()} ללא פריסה מתועדת${reasons.length ? " · " + reasons[0] : ""}`,
        noteEntries,
        notes: (dl.notes || []).join(" | ") || undefined,
        nextPaymentNote:
          !built.isCancelled && built.expectedOut > 0.5
            ? (dl.notes || [])
                .concat(built.payments.map((x) => x.note).filter(Boolean))
                .find((t) => /בהמשך|תשלומים|לחודש/.test(t || ""))
            : undefined,
      ...
      if (
        Math.abs((reg.totalPaid || 0) - built.collected) > 0.5 ||
        Math.abs((reg.outstanding || 0) - built.expectedOut) > 0.5 ||
        (!built.isCancelled && reg.paymentStatus !== built.expectedStatus)
      ) {
        mismatches.push(
          `${dl.dealId}: recompute שונה מהסימולציה (paid ${reg.totalPaid}/${built.collected}, out ${reg.outstanding}/${built.expectedOut}, status ${reg.paymentStatus}/${built.expectedStatus})`,
        );
      }
      ...
      reg.payments.forEach((pm) => {
        ...
        if (pr)
          sourceRefDocs.push({
            ...
            kind: "payment",
            sourceFile: pr[1],
            sourceSheet: SHEET_BY_FILE[pr[1].replace(".xlsx", "")],
            sourceRow: parseInt(pr[2], 10),
          });
      });
    }
  }
  ...
  console.log(
    `📄 נוצרו ${dealsCreated} עסקאות · ${paymentsCreated} תשלומים · ${sourceRefDocs.length} רפרנסים · ${writeOffs} עסקאות עם writeOff (הנחה/ביטול)`,
  );
  console.log(
    `📚 קורסים: ${coursesCreated} נוצרו, ${coursesEnriched} הועשרו (סה"כ ${courseByNorm.size})`,
  );
    ...
    for (const d of adv.deals || []) {
      await Expense.create({
        ...
        notes:
          [
            (d.notes || []).join(" | "),
            d.amountPaid != null && d.amountPaid !== d.amount
              ? `שולם: ₪${d.amountPaid}`
              : "",
          ]
            .filter(Boolean)
  ...
  const jsonPurchased = data.students.reduce(
    (a, s) => a + s.totals.purchased,
    0,
  );
  ...
  console.log(
    `סה"כ נרכש (ללא מבוטלות): DB=₪${aggActive.total.toLocaleString()}  JSON=₪${jsonPurchased.toLocaleString()}  ${ok2 ? "✓" : "✗ הפרש " + (aggActive.total - jsonPurchased)}`,
  );
  console.log(
    `סה"כ נגבה (כולל מבוטלות): DB=₪${aggAll.paid.toLocaleString()}  JSON=₪${jsonPaid.toLocaleString()}  ${ok3 ? "✓" : "✗ הפרש " + (aggAll.paid - jsonPaid)}`,
  );
  if (aggCancelled) {
    console.log(
      `מבוטלות: ${aggCancelled.n} עסקאות · מחיר ₪${aggCancelled.total.toLocaleString()} · נגבה ₪${aggCancelled.paid.toLocaleString()} · חוב פתוח ₪${aggCancelled.out.toLocaleString()} (חייב 0)`,
    );
    if (aggCancelled.out > 0.5)
      mismatches.push(`עסקאות מבוטלות עם חוב פתוח: ₪${aggCancelled.out}`);
  }
  const stDist = await Registration.aggregate([
    {
      $group: {
        _id: { t: "$recordType", s: "$paymentStatus" },
        n: { $sum: 1 },
      },
    },
  ]);
  console.log(
    "התפלגות:",
    stDist
      .map((x) => `${x._id.t}/${x._id.s}=${x.n}`)
      .sort()
      .join("  "),
  );

  if (!ok1 || !ok2 || !ok3)
    mismatches.push("אי-התאמה במספרים המצרפיים (ראו למעלה)");
