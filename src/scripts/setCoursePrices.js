...
const { buildCourseIndex, matchDealToCourse } =
  await import("../utils/courseMatch.js");
...
for (const c of courses) {
  ...
  console.log(
    `  ${c.name.padEnd(28)} → ₪${fp.price}  (מתוך ${fp.count}/${fp.n} עסקאות בתחום "${c.field}")`,
  );
  if (APPLY) {
...
if (skipped.length)
  console.log(`⚠️ no deal data (price left as-is): ${skipped.join(", ")}`);
