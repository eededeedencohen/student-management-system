...
const buildInstallmentItems = (plan, { dealDate, remaining }) => {
  ...
  const per = amount && amount > 0
    ? amount
    : remaining > 0 ? Math.round((remaining / count) * 100) / 100 : 0;
...
const nonEmpty = (row) => row && row.some((c) => c !== null && String(c).trim() !== '');

/** Build the payments array + derived money fields from raw deal columns. */
const buildMoney = ({ total, advance, method1, advDateRaw, balanceDue, paidDateRaw, method2, finalBalance, paidInFull, hasFuturePlan, sourceFile, sourceRow }) => {
  const payments = [];
  ...
  const totalPaid = payments.reduce((s, p) => s + p.amount, 0) || (advance > 0 ? advance : 0);
  ...
  return { payments, totalPaid, outstanding, paymentStatus, primaryCategory: m1.category, installments: m1.installments };
};
...
const run = async () => {
  ...
  console.log(`🧹  cleared prior import → reg:${del[0].deletedCount} course:${del[1].deletedCount} exp:${del[2].deletedCount} stud:${del[3].deletedCount}`);
  ...
  const manager = await upsertUser('עדן', 'manager', { email: 'eededeedencohen@gmail.com' });
  const moran = await upsertUser('מורן', 'rep', { aliases: ['מורן'] });
  const michal = await upsertUser('מיכל פרייס', 'rep', { aliases: ['מיכל', 'מיכל פרייס'] });
  console.log(`👤  users → manager:${manager.name}  reps: ${moran.name}, ${michal.name}`);
  ...
  if (courseDateFixes) console.log(`🩹  course end-date year typos corrected: ${courseDateFixes}`);

  const insertedCourses = courseDocs.length ? await Course.insertMany(courseDocs) : [];
  const courseByKey = new Map();
  insertedCourses.forEach((c) => courseByKey.set(`${c.field}|${c.cohortLabel}`, c._id));
  ...
  const processFile = ({ file, sheetName, rep, hasYear, cols, fallbackYear }) => {
    ...
    data.forEach((row, idx) => {
      ...
      const idNumber = cols.idNumber != null ? cleanStr(row[cols.idNumber]) : '';
      const hasAnyMoney = total > 0 || advance > 0 || balanceDue > 0 || finalBalance > 0;
      ...
      const PAID_FULL = /סיימ.? לשלם|סיים לשלם|אין( יותר)? יתרה|אין יתרת|הכל שולם|שולם הכל|היתרה שולמה|שולמה היתרה|שילמ.? הכל|אין חוב|שולם במלואו|שולמה במלואה/;

      let recordType = norm.recordType;
      let extraReview = norm.needsReview;
      let paidInFull = false;
      if (/החזר|ביטול/.test(idNumber) || total < 0 || advance < 0 || /^(?:החזר|ביטול|זיכוי)/.test(paidDateCell)) {
        // full cancellation / refund (negative money, or refund word in the id/paid column)
        recordType = 'refund';
        extraReview = true;
      } else if (/המשך/.test(idNumber) || /המשך/.test(courseRaw) || isInstallmentMarker) {
        ...
        if (/החזר|ביטול|זיכוי|מבטל/.test(`${paidDateCell} ${notesCell}`)) extraReview = true;
      }
      ...
      if (isNoisyNumber(cleanStr(row[cols.finalBalance]))) rowNeedsReview = true;

      const checklist = {
        signedTakanon: cols.takanon != null ? isYes(row[cols.takanon]) : false,
        addedToCourseWhatsapp: false,
        addedToAlumniWhatsapp: false,
        invoiceIssued: cols.invoice != null ? Boolean(cleanStr(row[cols.invoice])) : false,
      };
      ...
      registrations.push({
        ...
        rep: rep._id,
        repName: rep.name,
        registeredByRaw: cols.registeredBy != null ? cleanStr(row[cols.registeredBy]) : '',
        courseRaw,
        courseField: norm.field,
        cohortLabel: norm.cohortLabel,
        course: courseByKey.get(`${norm.field}|${norm.cohortLabel}`) || undefined,
        dealDate: dateRes.date,
        dealDateRaw: dateRes.raw,
        dateAssumed: Boolean(dateRes.assumedYear),
        totalAmount: dealTotal,
        amountExVat: cols.exVat != null ? parseNumber(row[cols.exVat]) || undefined : undefined,
        vatAmount: cols.vat != null ? parseNumber(row[cols.vat]) || undefined : undefined,
        advancePaid: advance,
        balanceDue,
        finalBalance,
    ...
    console.log(`   • ${file}: ${count} registrations parsed, ${skipped} empty/separator rows skipped`);
  };

  // Moran columns (sheet גיליון1) – no ID, no VAT, no year in dates
  const MORAN_YEAR = Number(process.env.SEED_MORAN_YEAR) || 2025;
  console.log(`ℹ️  מורן: שנה משוערת לתאריכים = ${MORAN_YEAR} (ניתן לשנות עם SEED_MORAN_YEAR)`);
  processFile({
    file: 'מורן.xlsx',
    sheetName: 'גיליון1',
    rep: moran,
    ...
    cols: {
      date: 0, name: 1, course: 2, total: 4, advance: 5, method1: 6, advDate: 7,
      balanceDue: 8, paidDate: 9, method2: 10, finalBalance: 11,
      idNumber: null, exVat: null, vat: null, invoice: null, registeredBy: null, takanon: null, notes: null,
    },
  });
  ...
  processFile({
    ...
    hasYear: true,
    cols: {
      date: 0, name: 1, idNumber: 2, course: 3, total: 4, advance: 5, method1: 6, advDate: 7,
      balanceDue: 8, paidDate: 9, method2: 10, finalBalance: 11, exVat: 12, vat: 13,
      invoice: 14, registeredBy: 15, takanon: 16, notes: 17,
    },
  });
  ...
  const allNames = [...new Set(registrations.map((r) => normName(r.studentName)).filter(Boolean))];
  ...
  for (const a of allNames) {
    ...
    if (ta.length >= 2) {
      for (const b of allNames) {
        if (b === a) continue;
        const tb = tokensOf(b);
        if (tb.length > tokensOf(best).length && ta.every((t) => tb.includes(t))) best = b;
      }
    }
    canonicalOf.set(a, best);
  }
  for (const r of registrations) {
    r._studentKey = canonicalOf.get(normName(r.studentName)) || normName(r.studentName) || null;
  }
  ...
  for (const g of snapGroups.values()) {
    ...
    g.sort(
      (a, b) =>
        ...
        (b.dealDate ? +new Date(b.dealDate) : 0) - (a.dealDate ? +new Date(a.dealDate) : 0)
    );
  ...
  for (const parent of registrations) {
    if (parent.recordType !== 'registration' || parent.outstanding <= 0.5) continue;
    ...
    for (const o of others) {
      const plan = parseInstallmentPlan(`${o.notes || ''} ${o.courseRaw || ''} ${o.nextPaymentNote || ''}`);
      if (plan) {
  ...
  for (const [key, arr] of markersByStudent) {
    ...
    const stamps = arr.map((x) => x.r.dealDate).filter(Boolean).map((d) => +new Date(d)).sort((a, b) => a - b);
    ...
    parent.installmentPlan = buildInstallmentItems(
      ...
      { dealDate: start, remaining: Math.max(parent.totalAmount - parent.advancePaid, 0) }
    );
  }
  ...
  for (const r of registrations) {
    ...
    // amount across them.
    const perAmt = (r.advancePaid > 0 ? r.advancePaid : 0) / marker.indices.length;
  ...
  const followupRows = registrations
    ...
    .sort((a, b) => (a.dealDate ? +new Date(a.dealDate) : 0) - (b.dealDate ? +new Date(b.dealDate) : 0));
  for (const r of followupRows) {
    ...
    const parent = dealPool.find((d) => (remainingOf.get(d) || 0) > 0.5) || dealPool[0];
    if (!parent) continue;
    ...
    if (rem !== Infinity && amt > rem + 0.5 && stated > 0 && stated < rem - 0.5) {
    ...
    parent.payments.push({
      ...
      source:
        ...
        (fold !== amt ? ` · סכום תוקן מ-${amt} ל-${Math.round(fold)} לפי שרשרת היתרה` : '') +
        (r.notes ? ' · ' + r.notes : ''),
    });
  ...
  for (const p of registrations) {
    if (p.recordType === 'registration') {
      const paidSum =
        p.payments.reduce((s, x) => s + (x.amount || 0), 0) || (p.advancePaid > 0 ? p.advancePaid : 0);
      p.totalPaid = p.totalAmount > 0 ? Math.min(paidSum, p.totalAmount) : paidSum;
      if (p.totalAmount > 0) p.outstanding = Math.max(p.totalAmount - p.totalPaid, 0);
      p.paymentStatus =
        p.totalAmount > 0 && p.outstanding <= 0.5 ? 'paid' : p.totalPaid > 0 ? 'partial' : 'unpaid';
  ...
  console.log(`📎  prose collection rows folded into parent deals: ${foldedFollowups} (typo-corrected via balance chain: ${typoCorrections})`);
  ...
  for (const p of registrations) {
    ...
    let note = '';
    if (paid > total + 1) note = `נגבה (${Math.round(paid)}) גדול מסה"כ (${Math.round(total)})`;
  ...
  console.log(`⚖️  reconciliation: ${registrations.filter((r) => r.recordType === 'registration').length - unbalanced} balanced, ${unbalanced} flagged`);
  const plannedCount = registrations.filter((r) => r.installmentPlan && r.installmentPlan.length).length;
  console.log(`🔗  installment plans: ${plannedCount} deals (${crossRowPlans} from follow-up rows) | markers linked: ${linkedMarkers}`);
  ...
  const cleanId = (id) => (id && !/[א-תA-Za-z]/.test(id) && /\d/.test(id) ? id : '');
  for (const r of registrations) {
  ...
  const insertedRegs = registrations.length ? await Registration.insertMany(registrations) : [];
  const insertedExpenses = expenses.length ? await Expense.insertMany(expenses) : [];
  ...
  console.log(`   ⚑ needsReview: ${needsReview}   |   ⚑ year-assumed (Moran): ${assumedYear}`);
  console.log(`   revenue (real deals only): ${sum(realRegs, (r) => r.totalAmount).toLocaleString()} ₪`);
  console.log(`   collected: ${Math.round(sum(realRegs, (r) => r.totalPaid)).toLocaleString()} ₪  |  outstanding: ${Math.round(sum(realRegs, (r) => r.outstanding)).toLocaleString()} ₪`);
  console.log('='.repeat(60));
