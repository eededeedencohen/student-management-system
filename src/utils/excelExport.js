...
const fitWidth = (header, values, { min = 10, max = 42 } = {}) => {
  const lens = [String(header || '').length * 1.2, ...values.map((v) => String(v ?? '').length)];
...
export const buildStyledWorkbook = ({
  ...
  if (subtitle) {
    ...
    const c = ws.getCell(rowIdx, 1);
    c.value = subtitle;
    c.font = { name: 'Arial', size: 10, italic: true, color: { argb: 'FF64748B' } };
    c.alignment = { horizontal: 'right', vertical: 'middle' };
    rowIdx += 1;
  }
  ...
  rows.forEach((r, i) => {
    ...
    columns.forEach((col, cI) => {
      ...
      if (type === 'money') {
        ...
        cell.value = v instanceof Date && !Number.isNaN(v.getTime()) ? v : v || '';
      ...
      if (type === 'status' && STATUS_STYLES[v]) {
        ...
        cell.font = { name: 'Arial', size: 11, bold: true, color: { argb: STATUS_STYLES[v].font } };
      }
    });
    rowIdx += 1;
  });
  ...
  if (totals.length && rows.length) {
    ...
    columns.forEach((col, cI) => {
      ...
      if (cI === 0) {
        ...
        cell.font = { name: 'Arial', size: 11, bold: true, color: { argb: BRAND_DARK } };
  ...
  ws.views = [
    { rightToLeft: true, state: 'frozen', ySplit: headerRowIdx, activeCell: 'A1' },
  ];
  ws.autoFilter = {
