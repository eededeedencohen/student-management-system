...
export const signOAuthState = (payload) =>
  jwt.sign({ ...payload, purpose: "gmail-oauth" }, STATE_SECRET(), {
    expiresIn: "15m",
  });
...
export const exchangeCodeForTokens = async ({ code, redirect }) => {
  ...
  if (!res.ok) {
    throw new Error(
      data.error_description || data.error || "החלפת קוד ההרשאה נכשלה",
    );
  }
  return {
...
export const refreshAccessToken = async (refreshToken) => {
  ...
  if (!res.ok) {
    ...
    const err = new Error(
      data.error_description || data.error || "רענון ההרשאה נכשל",
    );
    err.code = data.error;
    throw err;
  }
...
const filenameParam = (param, name) => {
  ...
  if (/^[\x20-\x7E]*$/.test(s))
    return `${param}="${s.replace(/(["\\])/g, "\\$1")}"`;
  const pct = encodeURIComponent(s).replace(
    /['()*!~]/g,
...
export const buildRawMessage = ({
  ...
  const files = Array.isArray(attachments)
    ? attachments.filter((a) => a && a.base64)
    : [];
  if (files.length === 0) {
    mime = `${headers.join("\r\n")}\r\nContent-Type: text/html; charset="UTF-8"\r\nContent-Transfer-Encoding: base64\r\n\r\n${wrap76(
    ...
    const boundary = `=_safra_${Buffer.from(String(files.length) + subject)
      .toString("base64url")
      .slice(0, 24)}`;
...
export const sendGmailMessage = async ({ accessToken, raw }) => {
  ...
  if (!res.ok) {
    const msg =
      data.error?.message || data.error_description || "שליחת המייל נכשלה";
